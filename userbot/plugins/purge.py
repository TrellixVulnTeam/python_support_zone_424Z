...
async def editer(edit):
    ...
    if LOGGER:
        await edit.client.send_message(
            LOGGER_ID, "Edit query was executed successfully"
        )
        ...
        try:
            ...
            if LOGGER:
                await delme.client.send_message(
                    LOGGER_ID, "Deletion of message was successful"
                )
...
async def fastpurger(purg):
    ...
    if LOGGER:
        await purg.client.send_message(
            LOGGER_ID, "Purge of " + str(count) + " messages done successfully."
        )
    await sleep(2)
    await done.delete()
    await purg.delete()
...
async def selfdestruct(destroy):
    ...
    await smsg.delete()
    if LOGGER:
        await destroy.client.send_message(LOGGER_ID, "sd query done successfully")
