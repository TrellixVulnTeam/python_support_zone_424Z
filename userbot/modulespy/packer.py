...
@PYTHONBOT.on(admin_cmd(pattern=r"unpack", outgoing=True))
@PYTHONBOT.on(sudo_cmd(pattern=r"unpack"))
async def _(event):
...
@PYTHONBOT.on(admin_cmd(pattern="repack ?(.*)", outgoing=True))
@PYTHONBOT.on(sudo_cmd(pattern="repack ?(.*)", allow_sudo=True))
async def _(event):
